...
#These classes allow for modification of the textures of objects of a given material

class solid_color:
    '''
    This is the standard texture, where the texture is just one color.
    Accordinlgy its value is just a rgb vector.
    '''
...
class checker_texture:
    '''
    This is an extension of the above texture, where the texture now takes two colors
    and uses a sine function to create a checkerboard pattern.
    '''
...
class perlin:
    '''
    This is a class used in the image_noise texture to generate perlin noise.
    '''
    def __init__(self):
...
class noise_texture:
    '''
    This calls the perlin noise class and turns it into a texture 
    that objects can use.
    '''
...
class image_texture:
    '''
    This class takes in a filename that points to an image in the image directory,
    and then wraps that image around the object.
    '''
