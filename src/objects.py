...
class xy_rect(hittable):
    '''
    This is a z-axis aligned rectangle specified by x and y points that determine its boundaries
...
class xz_rect(hittable):
    '''
    This is a y-axis aligned rectangle specified by x and z points that determine its boundaries
...
class yz_rect(hittable):
    '''
    This is a x-axis aligned rectangle specified by y and z points that determine its boundaries
...
class box(hittable):
    '''
    This uses the above rectangle classes to construct a box
    '''
...
class translate(hittable):
    '''
    This is not actually an object, it is an object instance
    It allows for you to translate the object to a different position just by 
    calculating the offset and testing the object with a new ray with a new 
    origin given by the offset.
    '''
...
class rotate_y:
    '''
    Similar to the translate class. this is not actually an object, it is an object instance.
    It allows for you to translate the object to a different position just by 
    calculating the new direction and origin of the ray based on the angle of rotation
     and testing the given object with this new ray.
    '''
...
class flip_face(hittable):
    '''
    This is another instance. It is used to flip the normal of the object,
    and is only really called when I want to change how light interacts with the environment
    (i.e. make it go in once specific direction).
    '''
...
class constant_medium:
    '''
    This class is an instance that takes in a standard object and turns it into an object
    of a constant medium (e.g. made out of smoke). It uses the isotropic material.
    It does this by allowing the ray to enter and scatter somehwere randomly inside the object
    rather than purely when interacting with the surface, which allows for some transparency,
    simulating smoke.
    '''
...
class hittable_list:
    '''
    This class is used to organize objects into a world. The ray_color function
    tests the world to see if any objects were hit, and these objects are organized in
    that world by a hittable list.
    '''
    def __init__(self):
        self.data = []
